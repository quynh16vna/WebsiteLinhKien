...
public partial class Admin_ProductCategoryList : System.Web.UI.Page
{
    ...
    public void LoadData()
    {
        ...
        ListBox_Category.SelectedValue = item.ProductCategoryID.ToSafetyString();
    ...
    public void LoadMainCategoryData()
    {
        DBEntities db = new DBEntities();
        //Query dữ liệu
        var query = db.ProductMainCategories.OrderBy(x => x.Position).Select(x => new { x.ProductMainCategoryID, x.Title });
        //Lấy danh sách query đó
        var data = query.ToList();
        //Cấu hình hiển thị
        DropDownList_MainCategory.DataValueField = "ProductMainCategoryID";//Khóa chính
        DropDownList_MainCategory.DataTextField = "Title";//Tên cột cần hiển thị

        DropDownList_MainCategoryLeft.DataValueField = "ProductMainCategoryID";//Khóa chính
    ...
    public void LoadCategoryData()
    {
        ...
        DBEntities db = new DBEntities();
        //Query dữ liệu
        var query = db.ProductCategories.OrderBy(x => x.Position).Where(x => x.ProductMainCategoryID == mID).Select(x => new { x.ProductCategoryID, x.Title });
        //Lấy danh sách query đó
        var data = query.ToList();
        //Cấu hình hiển thị
        ListBox_Category.DataValueField = "ProductCategoryID";//Khóa chính
