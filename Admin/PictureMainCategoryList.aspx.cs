...
public partial class Admin_PictureMainCategoryList : System.Web.UI.Page
{
    ...
    public void LoadCategoryData()
    {
        ...
        var query = db.PictureMainCategories.OrderBy(x => x.Position)
            .Select(x => new { x.PictureMainCategoryID, x.Title });
        var data = query.ToList();

        //Cấu hình hiển thị
        ListBox_Category.DataValueField = "PictureMainCategoryID";
        ...
        item.Text = ".:Chọn loại danh mục hình ảnh:.";
    ...
    public void LoadData()
    {
        ...
        DBEntities db = new DBEntities();
        var item = db.PictureMainCategories.Where(x => x.PictureMainCategoryID == id)
            .FirstOrDefault();
        if (item == null)
        {
            return;
        ...
        input_ID.Value = item.PictureMainCategoryID.ToSafetyString();
    ...
    public void SearchData(string type = "", string message = "")
    {
        ...
        string url = "~/Admin/PictureMainCategoryList.aspx?id={0}&messagetype={1}&message={2}";
    ...
    protected void LinkButton_Delete_Click(object sender, EventArgs e)
    {
        ...
        DBEntities db = new DBEntities();
        var item = db.PictureMainCategories.Where(x => x.PictureMainCategoryID == ID).FirstOrDefault();
    ...
    protected void LinkButton_Save_Click(object sender, EventArgs e)
    {
        ...
        if (FileUpload_Avatar.FileName != string.Empty)
        {
            ...
            uploadUtility.FolderSave = "~/fileuploads/PictureMainCategory";
        ...
        if (id > 0)
        {
            ...
            string url = "~/Admin/PictureMainCategoryList.aspx?mid={0}&messagetype={1}&message={2}";
        ...
        else
        {
            ...
            string url = "~/Admin/PictureMainCategoryList.aspx?messagetype={0}&message={1}&id={2}";
            url = url.StringFormat("success", "Đã thêm mới hình ảnh thành công", item.PictureMainCategoryID);
            //Chuyển về trang đích
            Response.Redirect(url);
        }
    }
